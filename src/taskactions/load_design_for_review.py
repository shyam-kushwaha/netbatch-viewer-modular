...
class TaskActionsMixin:
    ...
    def _show_load_design_popup(self, task):
        """Show popup window to configure NetBatch parameters and command for design review."""
        import subprocess
        import os
        import re
        ...
        # Get CA_command or construct default
        ca_command = task.get('CA_jobcommand','')
        
        # Create popup window
        popup = tk.Toplevel(self.root)
        popup.title(f"Load Design for Review - Task {task_id}")
        ...
        # Title
        title_label = ttk.Label(main_frame, text=f"Configure Design Review - {block}", 
                               font=('Arial', 12, 'bold'))
        ...
        ttk.Label(cmd_frame, text="CA Command:", font=('Arial', 9, 'bold')).pack(anchor='w', pady=(0, 5))
        ...
        def update_preview(*args):
            ...
            # Modify -output_log_file to add _review before .log
            modified_command = re.sub(r'(-output_log_file\s+\S*?)\.log', r'\1_review.log', command)
            
            if '-output_log_file' not in modified_command and modified_command:
                # If no -output_log_file exists, add it
                modified_command += f' -output_log_file {wardarea}/logs/{block}_review.log'
        ...
        def execute_command():
            ...
            try:
                ...
                cmd_log_file = f"{wardarea}/design_review_tcsh_cmd_{block}.log"
                try:
                    with open(cmd_log_file, "w") as f:
                        f.write("="*80 + "\n")
                        f.write(f"Design Review Launch Command for {block}\n")
                ...
                cmd = ['xterm', '-T', f'Design Review: {block}', '-e', tcsh_cmd]
                subprocess.Popen(cmd)
                
                self.update_status(f"Launched design review for task {task_id}", "success")
                logger.info(f"Design review launched for task {task_id} with command: {nbjob_cmd}")
                popup.destroy()
                
                
            except Exception as e:
                error_msg = f"Failed to launch design review: {str(e)}"
